...
class Detector(Node):
    ...
    def social_zone_callback(self, msg):
        try:
            ...
            if class_ID != None:
                if self.save_img:
                    width = int(float(w)*im0s.shape[0])
                    height = int(float(h)*im0s.shape[1])
                    x = int(float(x)*im0s.shape[0] - width/2)
                    y = int(float(y)*im0s.shape[1] - height/2)

                    cv2.rectangle(im0s, (x, y), (x+width, y+height), (255, 0, 0), 3)
